...
class TestSorn(unittest.TestCase):
    def test_runsorn(self):
        self.assertRaises(Exception, Simulator.simulate_sorn(inputs = simulation_inputs, phase='plasticity', matrices=matrices_dict, time_steps=2, noise=True))
        self.assertRaises(Exception, Trainer.train_sorn(inputs = gym_input, phase='plasticity', matrices=matrices_dict, epochs=1, noise=True))
        self.assertRaises(Exception, Trainer.train_sorn(inputs = sequence_input, phase='plasticity', matrices=matrices_dict, epochs=1, noise=True))
        self.assertRaises(Exception, Trainer.train_sorn(inputs = sequence_input, phase='training', matrices=matrices_dict, epochs=1, noise=True))
